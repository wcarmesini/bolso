...
- **O par é escolhido pelo conjunto, não pela ordem do arquivo.** Cada combinação possível (linha do extrato × lançamento já feito) recebe uma nota — distância entre as datas, palavras em comum nas descrições e o **número da parcela**, quando os dois lados o trazem — e os melhores pares são fixados primeiro, no arquivo inteiro. Do jeito antigo, a primeira linha escolhia primeiro e levava um lançamento que combinava muito mais com outra logo abaixo; as duas saíam erradas.
- **Parcela conta mais que tudo, quando existe.** Uma compra em dez vezes vira dez lançamentos idênticos: mesmo valor, mesma descrição e — pela regra do Bolso — a mesma competência, a data da compra. Só o número da parcela os separa, e ele vem do banco ("PARC 02/10" no texto, ou o campo do Open Finance) e fica guardado no lançamento. Por isso ele pesa mais que a data e a descrição juntas: sem ele, a linha da parcela 2 casava com a 10 e a pessoa não tinha como perceber. O número aparece ao lado do nome em **toda** a tela de conferir — nas opções de "Conciliar com…", no par já escolhido e na lista de "sem par" —, igual à lista de lançamentos.
...
- **Marcar várias e decidir de uma vez**: a caixa de cada linha (e a do cabeçalho do bloco) liga uma barra embaixo com as ações em massa, inclusive **definir a categoria das marcadas**. Vinte tarifas do mesmo banco viram uma categoria num gesto, em vez de vinte. A barra fica **colada no rodapé**, junto do botão de confirmar: com cinquenta linhas, ele estaria a uma rolagem de distância.
- **Duzentas linhas continuam respondendo na hora.** Cada linha é memoizada e recebe funções estáveis, e o mapa de quem já está conciliado viaja por referência em vez de propriedade — sem isso, um clique redesenhava a fila inteira. O seletor de par só ordena os candidatos quando alguém o abre. Medido no navegador com 150 linhas: **777 ms por clique antes, 81 ms depois**.
- **E continuam rolando.** Numa fila longa o gargalo deixa de ser o clique e passa a ser o **desenho**: o gravador do navegador mostrou a conferência pintando três vezes mais que a lista de lançamentos, porque cada linha tem muito mais coisa. Três coisas resolveram: `content-visibility: auto` nas linhas (o navegador pula o que está fora da tela, e a pintura caiu para um terço), nada de `backdrop-filter` em elemento grudado (é recalculado a cada quadro) e o bloco "sem par" mostrando oito de cada vez, porque ele é para conferir e dobrava o tamanho da página. Com 150 linhas na fila e 150 sem par, a pintura durante a rolagem caiu de **~4.100 ms para ~1.700 ms**.
...
| **Orçamentos** | Uma lista dos seus orçamentos: o em uso fica em destaque, com quantas pessoas e o convite pendente na própria linha. Renomear e convidar ficam no menu do orçamento; criar abre um diálogo. Nada de campo solto |
